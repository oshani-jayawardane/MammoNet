...
def build_autoencoder(input_shape, num_filters=16):
    ...
    _, p3 = encoder_block(p2, num_filters*4)
    _, p4 = encoder_block(p3, num_filters*8)
    
    # Bridge
    b = conv_block(p4, num_filters*16) 
    
    # Decoder
    d1 = decoder_block(b, num_filters*8)
    d2 = decoder_block(d1, num_filters*4)
...
def build_unet(input_shape, num_filters=16, num_classes=1):
    input_layer = Input(input_shape)
    ...
    d1 = decoder_block_for_unet(b1, s4, num_filters*8)
    d2 = decoder_block_for_unet(d1, s3, num_filters*4)
